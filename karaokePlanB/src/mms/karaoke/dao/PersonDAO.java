...
public class PersonDAO {
	...
	public ArrayList<Person> selectAllPersonList() {
		...
		try {
			...
			Person person = null;
			while (rs.next()) {
				person = new Person(rs.getInt("id"), rs.getString("name"), rs.getString("gender"),
						rs.getString("room"));
	...
	public Person selectOnePerson(int random) {
		...
		try {
			...
			if (rs.next()) {
				temp = new Person(rs.getInt("id"), rs.getString("name"), rs.getString("gender"), rs.getString("room"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
